...
%token <ival> NUMBER
%token <sval> ID
%token WHILE IF ELSE ASSIGN NEQ
...
condition:
      ID NEQ NUMBER    { $$ = (get_var_value($1) != $3); free($1); }
    | ID '<' NUMBER    { $$ = (get_var_value($1) < $3); free($1); }
;
